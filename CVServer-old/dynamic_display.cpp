...
void readme();
void labels_hash(string session_name, Mat img_object, double ratio, vector<vector<int> > &button_map, vector<string> &true_labels, vector<vector<double> > &button_center, double &avg_button_size);
...
int main(int argc, char** argv)
{
    ...
    for ( ; ; ) {
        try {
            ...
            string current_state = files[state_index].substr(0, files[state_index].size() - 9);
...
void labels_hash(string session_name, Mat img_object, double ratio, vector<vector<int> > &button_map, vector<string> &true_labels, vector<vector<double> > &button_center, double &avg_button_size) {
    ...
    char query[1000];
    sprintf(query, "SELECT x1, y1, x2, y2, label FROM object_buttons WHERE session=\"%s\"", session_name.c_str());
    cout << query << endl;
