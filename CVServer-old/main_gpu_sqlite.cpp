...
static const char *getenv_default(const char *name, const char *defval) {
...
#define refImageDirBase (std::string("/home/guoanhong/Desktop/ApplianceReader/WebServer/images_reference/") + session + std::string("-crop.jpg"))
#define imageDirBase "/home/guoanhong/Desktop/ApplianceReader/WebServer/images_video/"
#define session (getenv_default("VL_SESSION", "unknown"))
...
int main(int argc, char** argv)
{
    ...
    double ratio = img_object.rows > img_object.cols ? 800.0 / (double)img_object.rows : 800.0 / (double) img_object.cols;
    Size img_object_size(img_object.cols * ratio, img_object.rows * ratio); //(150, 445)
    resize(img_object, img_object, img_object_size);
    cout << "Img_object size: " << img_object.cols << " " << img_object.rows << endl;
...
void getImageToProcess() {
    ...
    sprintf(query, "SELECT image, id, imageactive, target FROM images_video WHERE session='%s' AND imageactive>0 AND id>%s ORDER BY id DESC", session, id.c_str());
