...
void skinColorFilter(Mat img_skin, Mat &img_skin_binary) {
    ...
    // Filter image color
    // Scalar skin_lower_threshold = *new Scalar(20, 100, 100); // (70, 60, 50);// (50, 100, 150);
    // Scalar skin_upper_threshold = *new Scalar(30, 255, 255); // (100, 90, 70);// (100, 160, 255);
    // inRange(hsv, skin_lower_threshold, skin_upper_threshold, img_skin_binary);

    Mat lower_red_hue_range;
    Mat upper_red_hue_range;
    inRange(hsv, Scalar(0, 100, 150), Scalar(5, 255, 255), lower_red_hue_range);
    inRange(hsv, Scalar(149, 100, 150), Scalar(179, 255, 255), upper_red_hue_range);
    img_skin_binary = lower_red_hue_range | upper_red_hue_range;
...
void findFingertipLocation(Mat img_skin_binary, int area, Mat H, double avg_button_size, vector<Point2f> &fingertip_location) {
    ...
    cout << "-- percentageOfArea = " << size_of_biggest_contour / area << endl << endl;
    
    try {
        if (size_of_biggest_contour > 0.002 * area && index_of_biggest_contour >= 0 && index_of_biggest_contour < contours.size()) {
            ...
            fingertip_location[0].y = fingertip_location[0].y + 15;
...
bool matchesCurrentStateGPU(GpuMat descriptors_scene_gpu, vector<KeyPoint> keypoints_scene, Mat curr_state_img_scene, GpuMat descriptors_curr_state_gpu, vector<KeyPoint> keypoints_curr_state, float benchmark_ratio, float match_expectation_ratio) 
{
    ...
    matcher.matchSingle(descriptors_curr_state_gpu, descriptors_scene_gpu, trainIdx, distance);
    vector<DMatch> matches;
...
bool matchesExpectedStateGPU(GpuMat descriptors_scene_gpu, vector<KeyPoint> keypoints_scene, GpuMat descriptors_expected_state_gpu, vector<KeyPoint> keypoints_expected_state, float benchmark_ratio, float match_expectation_ratio) {
    ...
    matcher.matchSingle(descriptors_expected_state_gpu, descriptors_scene_gpu, trainIdx, distance);
    vector<DMatch> matches;
    BFMatcher_GPU::matchDownload(trainIdx, distance, matches);
